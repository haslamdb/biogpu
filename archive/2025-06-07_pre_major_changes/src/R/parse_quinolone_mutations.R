...
# we first download the fluoroquinolone resistance files from MicroBIGG-E (https://www.ncbi.nlm.nih.gov/pathogens/microbigge//#Quinolone%20AND%20Point)
# I did this as individual organisms and must have manually combined them to get this single file: fluoroquinolone_ressitance_table.tsv


genes <- read.table("fluoroquinolone_resitance_table.tsv", 
                 header = TRUE, 
                 sep = "\t", 
                 comment.char = "#", 
                 fill = TRUE, 
...
write.csv(mutations_table, file = "Known_Quinolone_Changes.csv") # Because Claude doesn't like when I talk about mutations, I will call them changes from now on.
...
write.csv(allels_table, file = "Known_Efflux_Pump_Genes.csv")

...
// FIXED: Updated build_database_command with proper initialization sequence
bool build_database_command(const PipelineConfig& config) {
    ...
    // FIXED: Use stat() instead of std::filesystem for initial path validation
    struct stat path_stat;
    if (stat(config.genome_dir.c_str(), &path_stat) != 0) {
        std::cerr << "ERROR: Genome directory/file does not exist: " << config.genome_dir << std::endl;
        return false;
    }
    
    // Display capacity settings
    ...
    try {
        // Create parent directories recursively using mkdir
        std::string cmd = "mkdir -p \"" + config.output_path + "\"";
    ...
    try {
        ...
        // FIXED: Call initialize_cuda_context() AFTER creating the builder object
        std::cout << "Initializing CUDA context..." << std::endl;
        if (!builder->initialize_cuda_context()) {
            std::cerr << "ERROR: Failed to initialize CUDA context" << std::endl;
            return false;
        }
        ...
        // FIXED: Use safe file type detection
        bool is_fna_file = false;
        try {
            is_fna_file = is_concatenated_fna_file(config.genome_dir);
            std::cout << "File type detection: " << (is_fna_file ? "concatenated FNA file" : "directory") << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "ERROR in file type detection: " << e.what() << std::endl;
            builder.reset();
            return false;
        }
        
        // Build database based on input type
        if (is_fna_file) {
            std::cout << "Processing concatenated FNA file using streaming processor..." << std::endl;
            try {
                success = builder->build_database_from_streaming_fna(config.genome_dir, config.taxonomy_dir);
            } catch (const std::exception& e) {
                std::cerr << "ERROR during FNA processing: " << e.what() << std::endl;
                success = false;
            }
        } else {
            ...
            try {
                ...
                std::cerr << "ERROR during genome processing: " << e.what() << std::endl;
                success = false;
            }
        }

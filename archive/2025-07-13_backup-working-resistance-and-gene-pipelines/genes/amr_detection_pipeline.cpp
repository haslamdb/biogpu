...
void AMRDetectionPipeline::allocateGPUMemory() {
    ...
    // Minimizers (estimate ~1000 minimizers per read)
...
void AMRDetectionPipeline::updateCoverageStatsFromKallistoEM() {
    ...
    // First, we need to get the current state of coverage stats from the GPU
    // This includes the per-position counts which are only on the GPU.
    uint32_t num_genes = amr_db->getNumGenes();
    if (num_genes == 0) return;
    cudaMemcpy(h_coverage_stats.data(), d_coverage_stats,
               num_genes * sizeof(AMRCoverageStats), cudaMemcpyDeviceToHost);
    CHECK_CUDA_ERROR("copying coverage stats from device for EM update");

    // Now, update the host-side stats with the EM results
    for (const auto& [gene_id, info] : gene_abundance_info) {
        if (gene_id < h_coverage_stats.size()) {
            auto& stats = h_coverage_stats[gene_id];
            float original_reads = stats.total_reads;
            float em_reads = std::round(info.total_reads);

            // If the original read count was 0, we can't scale other metrics.
            // We'll just update the read count and let finalize handle the rest.
            if (original_reads > 0 && em_reads > 0) {
                 float scale_factor = em_reads / original_reads;
                 stats.total_bases_mapped = std::round(stats.total_bases_mapped * scale_factor);
            } else if (em_reads == 0) {
                stats.total_bases_mapped = 0;
            }
            // Always update the total_reads, as this is the primary output of EM.
            stats.total_reads = em_reads;

            // RPKM and TPM will be recalculated in finalizeCoverageStats,
            // so we don't need to set them here.
        }
    }

    // CRITICAL FIX: Copy the updated stats back to the GPU.
    // This ensures that finalizeCoverageStats uses the EM-updated read counts.
    cudaMemcpy(d_coverage_stats, h_coverage_stats.data(),
               num_genes * sizeof(AMRCoverageStats), cudaMemcpyHostToDevice);
    CHECK_CUDA_ERROR("copying EM-updated coverage stats to device");

    std::cout << "Coverage statistics updated on host and device with EM results." << std::endl;
}

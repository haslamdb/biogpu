...
```bash
  ...
  1. Script: scripts/setup_clean_database.sh runs the database building process
  2. Python Builder: src/python/build_clean_dynamic_database.py does the actual work

  The database builder:
  - Reads wildtype protein sequences from data/wildtype_protein_seqs/
  - Discovers species and genes by parsing FASTA headers
  - Looks for gene names like gyrA, gyrB, parC, parE, etc.
  - Assigns dynamic IDs to discovered species and genes
  - Creates protein k-mer index
  - Optionally loads known mutations from CSV

  Key finding: The species discovery happens at line 78 - it assigns species IDs based on sorted order of discovered species names. If E. coli sequences are in data/wildtype_protein_seqs/, they should be discovered and assigned an ID (not necessarily 562).
```
...
- **Resistance mappings**: Gene-species-position mappings for known mutations - not used in protein kmer generation (only WT kmers generated)

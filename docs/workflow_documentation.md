...
This command uses these defaults (all can be overridden):
- Protein sequences: `data/wildtype_protein_seqs` (use first arg to override)
- Output directory: `data/integrated_clean_db` (use second arg to override)
...
- Includes reverse complement k-mers (use `--no-rc` to disable)
...
python3 src/python/build_clean_dynamic_database.py \
    /custom/protein/dir \
    /custom/output/dir \
    --nucleotide-sequences /custom/nucleotide/dir \
    --mutations-csv data/quinolone_resistance_mutation_table.csv
```

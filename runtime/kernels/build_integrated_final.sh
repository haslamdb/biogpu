#!/bin/bash
# Final integrated build script

set -e

echo "Building Final Integrated BioGPU Pipeline..."

rm -rf build_final
mkdir -p build_final
cd build_final

# Create the integrated pipeline that demonstrates full functionality
cat > bio_gpu_pipeline.cpp << 'EOF'
#include <iostream>
#include <string>
#include <vector>
#include <filesystem>
...
# Compile
echo "Compiling..."
nvcc -o bio_gpu_pipeline bio_gpu_pipeline.cpp \
     -std=c++17 \
     -lcudart \
     -O3
...
if [ -f "bio_gpu_pipeline" ]; then
    ...
    echo -e "\nCopying to system location..."
    cp bio_gpu_pipeline ../build_unified/bio_gpu_pipeline
    echo "Updated: ../build_unified/bio_gpu_pipeline"
    echo -e "\nTest with:"
    ...
    echo "  --output-dir /tmp/test_output \\"

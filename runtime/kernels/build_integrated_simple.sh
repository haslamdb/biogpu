#!/bin/bash
# Simplified build script that compiles the integrated pipeline directly

set -e

echo "Building Integrated BioGPU Pipeline..."

# Clean build directory
rm -rf build_integrated_simple
mkdir -p build_integrated_simple
cd build_integrated_simple

# Create a simplified integrated pipeline that calls the existing executables
cat > bio_gpu_pipeline_integrated.cpp << 'EOF'
#include <iostream>
#include <string>
#include <vector>
#include <cstdlib>
...
g++ -o bio_gpu_pipeline bio_gpu_pipeline_integrated.cpp \
    -std=c++17 \

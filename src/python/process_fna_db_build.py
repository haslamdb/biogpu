#!/usr/bin/env python3
"""
Process .fna files for Kraken2 database construction.

This script:
1. Finds all .fna files in nested directories
2. Extracts assembly_accession from filenames
3. Looks up taxid and metadata from assembly_summary.txt
4. Modifies headers with assembly_accession and taxid
5. Moves processed files to output directory

Usage:
    python process_fna_files.py --library_dir /path/to/library --output_dir processed_fna
    python src/python/process_fna_db_build.py --library-dir ~/Data/NewK2Fasta/none/library --output_dir ~/Data/NewK2Fasta/processed_fna
"""
...
class FNAProcessor:
    def __init__(self, assembly_summary_path: str):
        """Initialize with assembly summary file."""
    ...
    def load_assembly_summary(self, summary_path: str) -> Dict[str, Dict]:
        ...
        try:
            ...
            df = pd.read_csv(summary_path, sep='\t', header=0, names=columns, 
                           dtype={'taxid': str, 'species_taxid': str, 'genome_size': str})
    ...
    def extract_assembly_accession(self, filename: str) -> Optional[str]:
        """Extract assembly accession from filename."""
        # Pattern: assembly_accession_genomic.fna
        # Example: GCF_037817735.1_ASM3781773v1_genomic.fna
        match = re.match(r'(GC[AF]_\d+\.\d+)_.*_genomic\.fna$', filename)
        if match:
            return match.group(1)
        return None
    
    def find_fna_files(self, input_dir: str) -> list:
        """Find all .fna files in directory and subdirectories."""
        fna_files = []
        input_path = Path(input_dir)
        
        for fna_file in input_path.rglob('*.fna'):
            if fna_file.is_file():
                fna_files.append(fna_file)
        
        logger.info(f"Found {len(fna_files)} .fna files")
        return fna_files
    ...
    def process_single_fna(self, fna_path: Path, output_dir: str) -> bool:
        ...
        assembly_accession = self.extract_assembly_accession(filename)
        
        if not assembly_accession:
            logger.warning(f"Could not extract assembly accession from {filename}")
            self.error_count += 1
            return False
        ...
        try:
            with open(fna_path, 'r') as infile, open(output_path, 'w') as outfile:
                for line in infile:
                    if line.startswith('>'):
                        # Modify header
    ...
    def process_library_subdirectories(self, library_dir: str, output_dir: str):
        ...
        for subdir in subdirs_to_process:
            ...
            if not fna_files:
                logger.warning(f"No .fna files found in {subdir.name}")
                continue
            
            logger.info(f"Processing {len(fna_files)} .fna files in {subdir.name}...")
...
def main():
    parser = argparse.ArgumentParser(description="Process .fna files for Kraken2 database")
    ...
    # Process files using the new subdirectory method
    processor = FNAProcessor("")  # Initialize with empty string since we'll load per subdirectory

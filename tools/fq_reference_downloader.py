...
class ResistanceGeneDownloader:
    ...
    def find_gene_by_tblastn(self, protein_seq: str, species: str, gene_name: str) -> Tuple[str, int, int, bool]:
        """Use tblastn to find gene location by searching protein against genome"""
        try:
            logger.info(f"Attempting tblastn search for {gene_name} in {species}")
            
            # Search for reference genome
            search_term = f"{species}[Organism] AND (complete genome[Title] OR chromosome[Title]) AND RefSeq[Filter]"
            search_handle = Entrez.esearch(db="nucleotide", term=search_term, retmax=5)
            search_results = Entrez.read(search_handle)
            search_handle.close()
            
            if not search_results["IdList"]:
                logger.warning(f"No reference genome found for {species}")
                return None, None, None, False
            
            # Note: This would require BLAST+ to be installed and configured
            # For now, we'll return None as this is a fallback method
            logger.info("tblastn search not implemented - would require BLAST+ setup")
            
        except Exception as e:
            logger.error(f"Error in tblastn search: {e}")
    ...
    def process_protein_accession(self, accession: str, species: str, gene_name: str, 
        ...
        try:
            ...
            # If this is a WP_ accession and we don't have nucleotide info, use special handling
            if accession.startswith("WP_") and not nucleotide_acc:
                logger.info(f"Using special handling for RefSeq protein {accession}")
                nucleotide_acc, gene_start, gene_end, is_complement = self.find_nucleotide_for_wp_accession(
                    accession, species, gene_name
                )
            
            # Use provided coordinates if available and we don't have them
            # Note: For WP_ accessions, the coordinates in the CSV might not be accurate
            if gene_info and not gene_start and not accession.startswith("WP_"):
    ...
    def download_mutation_gene_references(self):
        ...
        for key, mutations in mutations_by_key.items():
            ...
            if ref_gene:
                ...
                # Validate that the sequence has wild-type amino acids at mutation positions
                validation_errors = []
                for mut in mutations:
                    pos = mut['position'] - 1  # Convert to 0-based
                    expected_wt = mut['wild_type']
                    
                    if pos < len(ref_gene.protein_sequence):
                        actual_aa = ref_gene.protein_sequence[pos]
                        if actual_aa != expected_wt:
                            validation_errors.append(
                                f"Position {mut['position']}: expected {expected_wt}, found {actual_aa}"
                            )
                
                if validation_errors:
                    logger.warning(f"Validation issues for {species} {gene} ({protein_acc}):")
                    for error in validation_errors:
                        logger.warning(f"  - {error}")
                else:
                    logger.info(f"✓ Sequence validated: all wild-type amino acids match")
    ...
    def save_database(self, output_dir: str):
        ...
        # Save mutation positions for variant calling
        mutation_positions = {}
        for key, ref_gene in self.reference_genes.items():
            if ref_gene.mutations:
                mutation_positions[key] = {
                    'species': ref_gene.species,
                    'gene': ref_gene.gene_name,
                    'nucleotide_accession': ref_gene.nucleotide_accession,
                    'gene_start': ref_gene.gene_start,
                    'gene_end': ref_gene.gene_end,
                    'mutations': []
                }
                
                for mut in ref_gene.mutations:
                    # Calculate nucleotide position from amino acid position
                    aa_pos = mut['position']
                    nuc_offset = (aa_pos - 1) * 3  # 0-based nucleotide offset within gene
                    genome_pos = ref_gene.gene_start + nuc_offset
                    
                    mutation_positions[key]['mutations'].append({
                        'aa_position': aa_pos,
                        'nucleotide_position': genome_pos,
                        'wild_type_aa': mut['wild_type'],
                        'mutant_aa': mut['mutant'],
                        'codon_start': genome_pos,
                        'codon_end': genome_pos + 2
                    })
        
        with open(output_path / 'mutation_positions.json', 'w') as f:
            json.dump(mutation_positions, f, indent=2)
        ...
        stats = {
            ...
            'genes_per_type': {},
            'mutations_tracked': sum(len(r.mutations) for r in self.reference_genes.values() if r.mutations),
            'resistance_genes': sum(1 for r in self.reference_genes.values() if not r.mutations)
        }
